...
m_len <- length(MIC2)
train_seq <- seq(1, m_len, 2)
test_seq <- seq(2, m_len, 2)
train_part <- MIC2[train_seq]
test_part <- MIC2[test_seq]

trainY <- catego(train_part)
testY <- catego(test_part)
...
ctrlT <- trainControl(method = "repeatedcv", number = 5, repeats = 5,  verbose = TRUE, returnResamp = "all", savePredictions = "all", classProbs = TRUE, index = indexT)

testVars <- rfRFE$optVariables[1:5]
testData <- data.frame(testX[, testVars], testY)
modT <- train(testY ~., data = testData, trControl = ctrlT)
modT

# Get the prediction values in terms of probability, get  the call after prediction, get the
# actual class and the MIC for that sample, get this  for  all the repeatation

 testSample <- rownames(modT$trainingData)
 predSample <-  testSample[modT$pred$rowIndex]
 
 pred2  <- data.frame(predSample, modT$pred)
 
 
 MIC3 <- MIC1
 rownames(MIC3) <- paste0(rownames(MIC3), "_gent")
 predMIC <- MIC3[predSample,]
 ...
 #  Get the MICs for those pred  samples

lMIC <- log2(pred5$predMIC)
lMaxPred <- pred5$maxPred


colvar <- unlist(lapply(pred5$pred, function(x) {if (x == "S") "green" else "red"}))
