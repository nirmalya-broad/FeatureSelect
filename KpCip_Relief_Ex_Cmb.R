...
# This load the KpCip dataset and put in the appropriate format.

datadir <- '/home/nirmalya/research/DataDx/'
outDir <- paste0(datadir, '/KpCip_ReliefF_Ex/')
dir.create(outDir, showWarnings = FALSE)
datafile <- paste0(datadir, '/KpCip_for_Nirmalya_scaled.txt')

expData1 <- read.csv(datafile, sep = '\t', header = TRUE, row.names = 1, stringsAsFactors = FALSE)
labelData1 <- expData1[1, ]
labelData <- factor(unlist(labelData1))
MICData <- expData1[2,]
expData2 <- expData1[-(1:2),]
expData3 <- do.call('cbind', lapply(expData2, as.numeric))
rownames(expData3) <- rownames(expData2)
expData <- expData3
...
MIC1 <- MICData
m_len <- length(MIC1)
train_part <- seq(1, m_len, 2)
test_part <- seq(2, m_len, 2)
...
#trainY <- labelData[names(ex_part)]
#testY <- labelData[names(m_part)]

trainY <- labelData[train_part]
testY <- labelData[test_part]

# The data is now ready 

xdata1 <- data.frame(t(expData))
ydata <- trainY
